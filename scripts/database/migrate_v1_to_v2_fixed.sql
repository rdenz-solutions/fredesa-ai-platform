-- Migration: v1 → v2.1 (FIXED)
-- Safe migration that preserves existing data

BEGIN;

-- ============================================================================
-- STEP 1: Enhance CATEGORIES table
-- ============================================================================

-- Rename existing columns to match v2.1
...
-- Add display_name (copy from name)
...
-- Note: Won't add constraint until sources are classified
...
-- ============================================================================
-- STEP 2: Enhance SOURCES table (25+ new fields)
-- ============================================================================
...
-- ============================================================================
-- STEP 4: Quality Validation tables
-- ============================================================================
...
-- ============================================================================
-- STEP 5: Federated Architecture tables
-- ============================================================================
...
-- Category statistics auto-update (disabled until sources classified)
CREATE OR REPLACE FUNCTION update_category_epistemological_stats()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'INSERT' AND NEW.epistemological_dimension IS NOT NULL THEN
    ...
    ELSIF TG_OP = 'UPDATE' AND (OLD.epistemological_dimension IS NULL OR NEW.epistemological_dimension IS NULL) THEN
        -- Skip if either old or new dimension is NULL
        RETURN NEW;
    ELSIF TG_OP = 'UPDATE' AND (OLD.category_id != NEW.category_id OR OLD.epistemological_dimension != NEW.epistemological_dimension) THEN
        ...
        UPDATE categories SET
            ...
            future_sources = future_sources + CASE WHEN NEW.epistemological_dimension = 'future' THEN 1 ELSE 0 END
        WHERE id = NEW.category_id;
    ELSIF TG_OP = 'DELETE' AND OLD.epistemological_dimension IS NOT NULL THEN
        UPDATE categories SET
            total_sources = total_sources - 1,
            theory_sources = theory_sources - CASE WHEN OLD.epistemological_dimension = 'theory' THEN 1 ELSE 0 END,
            practice_sources = practice_sources - CASE WHEN OLD.epistemological_dimension = 'practice' THEN 1 ELSE 0 END,
...
-- Note: Trigger commented out until sources are classified
